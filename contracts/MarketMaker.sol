...
contract MarketMaker is EIP712MetaTransaction, Ownable, ERC1155TokenReceiver {
    ...
    modifier onlyWhitelisted() {
        require(
            whitelist == Whitelist(0) || whitelist.isWhitelisted(msgSender()),
        ...
        atStage(Stage.Paused)
    {
        ...
        if (fundingChange > 0) {
            require(collateralToken.transferFrom(msgSender(), address(this), uint(fundingChange)) && collateralToken.approve(address(pmSystem), uint(fundingChange)));
        ...
        returns (int netCost)
    {
        ...
        if(outcomeTokenNetCost > 0) {
            require(
                collateralToken.transferFrom(msgSender(), address(this), uint(netCost)) &&
        ...
        if(touched) pmSystem.safeBatchTransferFrom(msgSender(), address(this), positionIds, transferAmounts, "");

        if(outcomeTokenNetCost < 0) {
            mergePositionsThroughAllConditions(uint(-outcomeTokenNetCost));
        }

        emit AMMOutcomeTokenTrade(msgSender(), outcomeTokenAmounts, outcomeTokenNetCost, uint(fees));
        ...
        if(touched) pmSystem.safeBatchTransferFrom(address(this), msgSender(), positionIds, transferAmounts, "");

        if(netCost < 0) {
            require(collateralToken.transfer(msgSender(), uint(-netCost)));
        }
    }
