...
contract LMSRMarketMakerFactory is EIP712MetaTransaction, ConstructedCloneFactory, LMSRMarketMakerData {
        ...
        returns (LMSRMarketMaker lmsrMarketMaker)
    {
        ...
        collateralToken.transferFrom(msgSender(), address(this), funding);
        collateralToken.approve(address(lmsrMarketMaker), funding);
        lmsrMarketMaker.changeFunding(int(funding));
        lmsrMarketMaker.resume();
        lmsrMarketMaker.transferOwnership(msgSender());
        emit LMSRMarketMakerCreation(msgSender(), lmsrMarketMaker, pmSystem, collateralToken, conditionIds, fee, funding);
    }
}
