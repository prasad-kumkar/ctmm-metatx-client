...
contract ConditionalTokens is EIP712MetaTransaction, ERC1155 {
    ...
    function reportPayouts(bytes32 questionId, uint[] calldata payouts) external {
        ...
        bytes32 conditionId = CTHelpers.getConditionId(msgSender(), questionId, outcomeSlotCount);
        ...
        emit ConditionResolution(conditionId, msgSender(), questionId, outcomeSlotCount, payoutNumerators[conditionId]);
    }
    ...
    function splitPosition(
        ...
        if (freeIndexSet == 0) {
            ...
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transferFrom(msgSender(), address(this), amount), "could not receive collateral tokens");
            } else {
                _burn(
                    msgSender(),
        ...
        emit PositionSplit(msgSender(), collateralToken, parentCollectionId, conditionId, partition, amount);
    }
    ...
    function mergePositions(
        ...
        if (freeIndexSet == 0) {
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transfer(msgSender(), amount), "could not send collateral tokens");
            } else {
                _mint(
                    msgSender(),
                    CTHelpers.getPositionId(collateralToken, parentCollectionId),
        ...
        emit PositionsMerge(msgSender(), collateralToken, parentCollectionId, conditionId, partition, amount);
    }
    ...
    function redeemPositions(IERC20 collateralToken, bytes32 parentCollectionId, bytes32 conditionId, uint[] calldata indexSets) external {
        ...
        for (uint i = 0; i < indexSets.length; i++) {
            ...
            uint payoutStake = balanceOf(msgSender(), positionId);
            if (payoutStake > 0) {
                totalPayout = totalPayout.add(payoutStake.mul(payoutNumerator).div(den));
                _burn(msgSender(), positionId, payoutStake);
            }
        }

        if (totalPayout > 0) {
            if (parentCollectionId == bytes32(0)) {
                require(collateralToken.transfer(msgSender(), totalPayout), "could not transfer payout to message sender");
            } else {
                _mint(msgSender(), CTHelpers.getPositionId(collateralToken, parentCollectionId), totalPayout, "");
            }
        }
        emit PayoutRedemption(msgSender(), collateralToken, parentCollectionId, conditionId, indexSets, totalPayout);
    }
