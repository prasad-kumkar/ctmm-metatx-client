...
contract FPMMDeterministicFactory is EIP712MetaTransaction, Create2CloneFactory, FixedProductMarketMakerData, ERC1155TokenReceiver {
    ...
    function onERC1155Received(
        ...
        returns (bytes4)
    {
        ConditionalTokens(msgSender()).safeTransferFrom(address(this), currentFunder, id, value, data);
    ...
    function onERC1155BatchReceived(
        ...
        returns (bytes4)
    {
        ConditionalTokens(msgSender()).safeBatchTransferFrom(address(this), currentFunder, ids, values, data);
        ...
        returns (FixedProductMarketMaker)
    {
        ...
        if (initialFunds > 0) {
            currentFunder = msgSender();
            collateralToken.transferFrom(msgSender(), address(this), initialFunds);
            ...
            fixedProductMarketMaker.transfer(msgSender(), fixedProductMarketMaker.balanceOf(address(this)));
