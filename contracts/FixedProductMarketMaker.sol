...
contract FixedProductMarketMaker is EIP712MetaTransaction, ERC20, ERC1155TokenReceiver {
        ...
        external
    {
        ...
        require(collateralToken.transferFrom(msgSender(), address(this), addedFunds), "funding transfer failed");
        ...
        _mint(msgSender(), mintAmount);

        conditionalTokens.safeBatchTransferFrom(address(this), msgSender(), positionIds, sendBackAmounts, "");
        ...
        emit FPMMFundingAdded(msgSender(), sendBackAmounts, mintAmount);
    }
        ...
        external
    {
        ...
        _burn(msgSender(), sharesToBurn);
        collateralRemovedFromFeePool = collateralRemovedFromFeePool.sub(
            collateralToken.balanceOf(address(this))
        );

        conditionalTokens.safeBatchTransferFrom(address(this), msgSender(), positionIds, sendAmounts, "");

        emit FPMMFundingRemoved(msgSender(), sendAmounts, collateralRemovedFromFeePool, sharesToBurn);
    }
    ...
    function buy(uint investmentAmount, uint outcomeIndex, uint minOutcomeTokensToBuy) external {
        ...
        require(collateralToken.transferFrom(msgSender(), address(this), investmentAmount), "cost transfer failed");
        ...
        conditionalTokens.safeTransferFrom(address(this), msgSender(), positionIds[outcomeIndex], outcomeTokensToBuy, "");

        emit FPMMBuy(msgSender(), investmentAmount, feeAmount, outcomeIndex, outcomeTokensToBuy);
    }
    ...
    function sell(uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell) external {
        ...
        conditionalTokens.safeTransferFrom(msgSender(), address(this), positionIds[outcomeIndex], outcomeTokensToSell, "");
        ...
        require(collateralToken.transfer(msgSender(), returnAmount), "return transfer failed");

        emit FPMMSell(msgSender(), returnAmount, feeAmount, outcomeIndex, outcomeTokensToSell);
    }
}
